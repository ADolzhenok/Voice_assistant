...
def time(mongo_collect, user):
    outcome = speak_rus(f'Сейчас {strftime("%X")}')
    mongo_collect.insert_one(
...
def change_keyboard_lang(mongo_collect, user):
    ...
    mongo_collect.insert_one(
        {"user": user, "action": "Изменение языка раскладки",
         "details": [
             {"[ACTION]": "ИЗМЕНИТЬ ЯЗЫК РАСКЛАДКИ"},
             {"[TIME]": f'{strftime("%c")}'}
         ],
         "status": "SUCCESS",
         "outcome": f'{outcome}'
...
def volume(action, mongo_collect, user):
    ...
    mongo_collect.insert_one(
        {"user": user, "action": "Изменение громкости",
         "details": [
             {"[ACTION]": "ИЗМЕНИТЬ ГРОМКОСТЬ"},
             {"[TIME]": f'{strftime("%c")}'},
             {"[TYPE OF CHANGING]": f'{action}'}
         ],
         "status": "SUCCESS",
         "outcome": f'{outcome}'
         }
