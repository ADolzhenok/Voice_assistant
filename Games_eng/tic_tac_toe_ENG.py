...
def playerMove():
    run = True
    while run:
        speak_eng('Please select a position to place an \'X\' (1-9)')
        move = get_audio_eng()
        for char in move:
            if char.isdigit():
                move = char
...
def main():
    speak_eng('Welcome to Tic Tac Toe! Look at the screen')
    printBoard(board)
    ...
    while not (isBoardFull(board)):
        ...
        if not (isWinner(board, 'X')):
            ...
            if move == 0:
                ...
                insertLetter('O', move)
                speak_eng(f'Computer placed an \'O\' in position {move}')
                printBoard(board)
        else:
            speak_eng('X\'s won this time! Good Job!')
            break
